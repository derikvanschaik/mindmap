#A basic PySimpleGUI graph example
import PySimpleGUI as sg
import random
import webbrowser
#This is the new line i added while checking out branch new

# -------------CONSTANTS BELOW---------------------#
img = r''
TEXT_COLORS = ['white', 'black']
BOX_COLORS = ['red', 'blue', 'green', 'yellow', 'purple', 'orange', 'pink']
BOX_COLORS_TO_TEXT = {
'red':'white', 'green':'white', 'purple':'white', 'blue':'white',
'orange':'black', 'pink':'black', 'yellow':'black'
					 }

MAX_X, MAX_Y = 1800, 1800 
BACKGROUND_COLOR = "gainsboro"
URLS = [
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=beaming",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=beautiful",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=believe",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=beneficial",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=bliss",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=bountiful",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=bounty",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=brave",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=bravo",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=brilliant",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=bubbly",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=calm",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=celebrated",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=certain",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=champ",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=champion",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=charming",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=cheery",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=choice",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=classic",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=classical",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=clean",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=commend",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=composed",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=congratulation",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=constant",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=cool",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=courageous",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=creative",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=cute",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=dazzling",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=delight",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=delightful",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=distinguished",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=divine",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=earnest",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=easy",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=ecstatic",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=effective",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=effervescent",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=efficient",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=effortless",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=electrifying",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=elegant",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=enchanting",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=encouraging",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=endorsed",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=energetic",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=energized",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=engaging",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=enthusiastic",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=essential",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=esteemed",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=ethical",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=excellent",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=exciting",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=exquisite",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=fabulous",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=fair",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=familiar",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=famous",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=fantastic",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=favorable",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=fetching",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=fine",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=fitting",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=flourishing",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=fortunate",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=free",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=fresh",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=friendly",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=fun",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=funny",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=generous",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=genius",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=genuine",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=giving",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=glamorous",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=glowing",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=good",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=gorgeous",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=graceful",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=great",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=green",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=grin",
"https://www.gettyimages.ca/search/2/image?family=creative&phrase=growing",
]

T_OFF = b''
T_ON = b''
# -------------CONSTANTS ABOVE---------------------#

def colored_button(color, text_color):
	return sg.B('', key=color, size=(5,1), pad=(0,0), button_color=(text_color, color) )

def color_palette():
	black = ('yellow', 'pink', 'orange')
	return [ colored_button(color, 'black') if color in black else colored_button(color, 'white') for color in BOX_COLORS]

def color_identifier_palette():
	row = [] 
	for color in BOX_COLORS:
		text= ''
		if color == 'red':
			text = '▼'
		row.append(sg.T(text, key=f'-TXT-{color}-', size=(6,1), pad=(0,0), background_color=BACKGROUND_COLOR))
	return row

def get_user_text(text_input_element, cursor_color):
	text_input_element.update("")
	text_input_element.Widget.config(insertbackground=cursor_color) #for the cursor to not be black 
	text_input_element.set_focus()

def clear_text(d1,d2,d3, canvas):
	for figure in [d1,d2,d3]:
		canvas.delete_figure(figure)

def draw_text(text, loc, txt_color, canvas):
	return canvas.draw_text( text = text, location = loc, color = txt_color, font = "default 13")

def keep_text_on_canvas(figure_ids, max_col, canvas):
	coordinate = canvas.get_bounding_box(figure_ids[0])[0]
	x = coordinate[0]
	x_delta = None

	if x <= 0:
		x_delta = -x

	elif x>=max_col:
		x_delta = -(x-max_col)
	
	if x_delta:
		for figure in figure_ids:
			canvas.move_figure(figure, x_delta, 0)

def write_text_to_canvas(already_is_text, draw_id_text, rect_id, draw_id, text, selected_area, cur_txt_color,cur_box_color, canvas):
	if already_is_text: # if user is currently writing text -deletes and updates text
		clear_text(draw_id, draw_id_text, rect_id, canvas = canvas) 

	draw_id_text = canvas.draw_text(text = text , location =selected_area, color=cur_txt_color, font='Default 13')
	rect_id = canvas.draw_rectangle( canvas.get_bounding_box(draw_id_text)[0] , canvas.get_bounding_box(draw_id_text)[1], fill_color=cur_box_color)
	draw_id = canvas.draw_text(text = text , location =selected_area, color=cur_txt_color, font='Default 13')
	keep_text_on_canvas([draw_id, rect_id], MAX_X, canvas)
	return draw_id_text, rect_id, draw_id

def connect_selected_text_boxes(selected_boxes, selection_cursors, canvas):
	if len(selected_boxes) >= 2:
		line_id = canvas.draw_line(selected_boxes[-1], selected_boxes[-2], width=2)
		canvas.SendFigureToBack(line_id)

	for cursor in selection_cursors: 
		canvas.delete_figure(cursor)
	selection_cursors.clear()

def update_color(chosen_color, text_color_dict):
	return chosen_color, text_color_dict[chosen_color]

def create_canvas(key):
	return sg.Graph((5000,5000), (0,0),(MAX_X,MAX_Y),background_color="white", enable_events=True, key=key)

def create_text_element(canvas, text, selected_area, cur_txt_color, additional_font):
	font_add = ""
	if additional_font:
		font_add += additional_font

	canvas.draw_text(text = text , location =selected_area, color=cur_txt_color, font='Default 13'+ ' ' +font_add)

def is_approx_in(search_tup, dict):
	y_threshold = 7
	x_threshold = 25
	for loc_tup in dict.keys():
		if search_tup[0] == loc_tup[0] and abs(search_tup[1] - loc_tup[1]) <= x_threshold and abs(search_tup[2] - loc_tup[2]) <= y_threshold:
			return loc_tup

	return None #could not find a match


def main():

	sg.theme("Material2")

	layout = [ 

   # [sg.Checkbox('Connect Mode', enable_events=True, key='Connect Mode', background_color=BACKGROUND_COLOR),\
	[sg.Column(
		[
		[sg.T('Connect Mode', font='default 9 italic', background_color=BACKGROUND_COLOR), sg.T('OFF', key='-CON-STAT-', text_color = 'dark grey', size=(3,1),background_color=BACKGROUND_COLOR,justification='right'), sg.T('', size=(12,1),  background_color=BACKGROUND_COLOR), sg.T('Links are:', font=('default 9 italic'),  background_color=BACKGROUND_COLOR), sg.T('HOT', text_color='red', k='-LINK-STAT-', size= (6,1),background_color=BACKGROUND_COLOR)],
		[sg.B(image_data=T_OFF, k='-TOGGLE1-', metadata=False, button_color=(BACKGROUND_COLOR, BACKGROUND_COLOR), border_width=0, image_subsample=2), sg.Button('Connect', size=(8,1)), sg.T('', size=(10,1), background_color=BACKGROUND_COLOR),sg.B('Link', size=(8,1)), sg.B(' ', button_color = ('red', 'red'), k='-HOT-LINK-',metadata=True, size=(5,1) ,pad=(0,0)), sg.B(' ', button_color = ('grey', 'grey'), k='-COLD-LINK-', metadata = False, size=(1,1), pad=(0,0)),\
		sg.T('', background_color = BACKGROUND_COLOR), sg.T('', size=(17,1), background_color=BACKGROUND_COLOR), sg.Input('image search', font=("default 13 italic"), size=(15,20),text_color='green', background_color = 'white', key = '-IMAGE-SEARCH-', pad=(0,0)), sg.B('☛ GO', button_color = ('white', 'green'),key='Go To Images', size=(8,1),pad=(0,0))]],\
		 background_color = BACKGROUND_COLOR), \
	
	sg.T('', size=(10, 1), background_color=BACKGROUND_COLOR), sg.Text('Colors:', background_color=BACKGROUND_COLOR)] + [sg.Column([color_identifier_palette(), color_palette()], background_color=BACKGROUND_COLOR)], #canvas change here

	[sg.TabGroup([[sg.Tab(f"Tab {i} ", [[sg.Column([[ create_canvas(i) ]], size=(1350,575),scrollable=True) ]], key = f"-TAB-{i}-", visible=(i==1)) for i in range(1,30) ]] + [[sg.Button('➕', key='-NEW-TAB-')]]) ] ,
				[sg.Input('', key='-IN-', enable_events=True, text_color=BACKGROUND_COLOR, background_color=BACKGROUND_COLOR)]
	]

	window = sg.Window('MIMI', layout, background_color=BACKGROUND_COLOR).finalize()

	window['-IMAGE-SEARCH-'].Widget.config(insertbackground='green') 
	 
	selected_area = (0,0)
	last_selected = (0,0)
	draw_id = None
	point_id = None
	connect = [] #will contain locations of boxes that are to be connected
	selected = [] #will contain loc of temporary selection cursors when in connect mode
	linked = {} #(tab num, x, y) --> link (tab number)
	linked_to_text = {} #(tab num, x, y) --> text 
	cur_txt_color = 'white'
	cur_box_color = 'red'
	cur_icon = '-TXT-red-'
	cur_canvas = 1
	most_recently_visible_canvas = 1
	linked_was_generated = False
	last_tab = 1
	connect_mode = False
	hot_links = True

	while True:
		event, values = window.read()

		if event==sg.WIN_CLOSED:
			break


		elif isinstance(event, int): #canvas was clicked
			cur_canvas = event
			get_user_text(window['-IN-'], BACKGROUND_COLOR)
			selected_area = values[event]
			draw_id = None 
			rect_id = None
			draw_id_text = None
			is_hot_link = is_approx_in( (event, selected_area[0], selected_area[1] ) ,linked)

			if connect_mode:
				if not selected_area in connect:
					connect.append(selected_area)
				selected.append(window[cur_canvas].draw_image(data=img, location=(selected_area[0]-10, selected_area[1]+8))) #draws snipping cursor

				if len(selected) > 2: # cannot connect more than 2 text boxes at a time -- delete least recent snipping cursor
					unused_cursor = selected.pop(0) 
					window[cur_canvas].delete_figure(unused_cursor)

			if is_hot_link and hot_links:
				window[f'-TAB-{linked[is_hot_link]}-'].select() #go to the hot link

			if is_hot_link and not hot_links and not connect_mode: #they are trying to go back to a link but forgot to enable hot links again 
				sg.popup("Hot links are disabled!")
			
		elif event=='-IN-':
			linked_to_text[(cur_canvas, selected_area[0], selected_area[1])] = values['-IN-'] 
			draw_id_text, rect_id, draw_id = write_text_to_canvas( all([draw_id, window['-IN-']]), draw_id, draw_id_text, rect_id, values['-IN-'].upper(), selected_area, cur_txt_color,cur_box_color, window[cur_canvas])
			#get the text associated with this tab number and location 
			# print(values['-IN-'])
			# print("selected area:", selected_area)

		elif event=='Connect':
			connect_selected_text_boxes(connect, selected, window[cur_canvas])

		elif event in BOX_COLORS: #updating textbox colors 
			cur_box_color, cur_txt_color = update_color(event, BOX_COLORS_TO_TEXT)
			window[cur_icon].update("")
			cur_icon = f'-TXT-{event}-'
			window[cur_icon].update("▼")

		elif event=='Go To Images':
			if values['-IMAGE-SEARCH-']:
				user_keyword = values['-IMAGE-SEARCH-']
				url = "https://www.gettyimages.ca/photos/" + user_keyword
				webbrowser.open(url) 
			else:
				webbrowser.open(random.choice(URLS))
			window['-IMAGE-SEARCH-'].update("")
			

		elif event == '-NEW-TAB-':
			print("most_recently_visible_canvas in new tab event", most_recently_visible_canvas)
			most_recently_visible_canvas += 1
			window[f'-TAB-{most_recently_visible_canvas}-'].update(visible=True)
			window[f'-TAB-{most_recently_visible_canvas}-'].select()

			if linked_was_generated:
				linked_was_generated = False
				create_text_element(window[most_recently_visible_canvas], linked_to_text[ (cur_canvas, selected_area[0], selected_area[1]) ], (50, 1780), 'blue', 'italic underline' )
				linked[ (most_recently_visible_canvas, 50, 1780 ) ] = cur_canvas # back link is created 

	 
		elif event == 'Link':

			tab_and_location_tup = (cur_canvas, selected_area[0], selected_area[1]) 
			linked[tab_and_location_tup] = most_recently_visible_canvas + 1 #forward link is created 
			window['-NEW-TAB-'].click() #activate new tab event
			linked_was_generated = True #identifier to draw a link

		elif event.startswith('-TOGGLE'): #Thanks for this one @ Mike! Taken from: https://github.com/PySimpleGUI/PySimpleGUI/blob/master/DemoPrograms/Demo_Simple_Material_Feel.py
			state = window[event].metadata = not window[event].metadata
			window[event].update(image_data=T_ON if state else T_OFF, image_subsample=2)
			window[event].set_size(size=(40,30))
			connect_mode = not connect_mode
			if connect_mode:
				window['-CON-STAT-'].update('ON', text_color='dark blue')
			else:
				window['-CON-STAT-'].update('OFF', text_color='dark grey')

		elif event in ('-HOT-LINK-', '-COLD-LINK-'):

			if hot_links:
				window['-HOT-LINK-'].update( button_color=('grey', 'grey'))
				window['-HOT-LINK-'].set_size(size=(1,1))
				window['-COLD-LINK-'].update(button_color=('blue', 'blue'))
				window['-COLD-LINK-'].set_size(size=(5,1))
				window['-LINK-STAT-'].update("COLD", text_color='blue')
			else:
				window['-HOT-LINK-'].update(button_color=('red', 'red'))
				window['-HOT-LINK-'].set_size(size=(5,1))
				window['-COLD-LINK-'].update(button_color=('grey', 'grey'))
				window['-COLD-LINK-'].set_size(size=(1,1))
				window['-LINK-STAT-'].update("HOT", text_color='red')

			hot_links = not hot_links




		# print(linked)
	window.close()

if __name__ == '__main__':
	main()