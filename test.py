...
def main():
    sg.theme('Material1')
    layout = [
          [sg.B('CONNECT ON', button_color = ('blue',"gainsboro"), pad=(0,0), border_width=0),sg.B('CONNECT OFF', button_color = ('red',"red"), pad=(0,0),  border_width=0)],
          [sg.Text("hello there")]
                       ]
    ...
    while True:
        ...
        if event == sg.WIN_CLOSED:
            break
        if any(list(map(lambda mode: event.endswith(mode), ('ON', 'OFF')))):
            if on:
                window['CONNECT ON'].update(button_color=('black',"black"))
                window['CONNECT OFF'].update(button_color=('blue',"gainsboro"))
            else:
                window['CONNECT ON'].update(button_color=('blue',"gainsboro"))
                window['CONNECT OFF'].update(button_color = ('red',"red"))
